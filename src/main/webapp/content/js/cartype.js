var carTypeTableEditable = function() {
	...
	var handleTable = function() {
		...
		function editRow(oTable, nRow) {
			...
			jqTds[0].innerHTML = '<input type="text" style="width: 100% !important;" class="form-control" value="'
					+ aData[1] + '">';
			
			var checked = "";			
			if(aData[2] && aData[2].indexOf && aData[2].indexOf('checked="true"') > 0)
			...
			jqTds[2].innerHTML = '<input type="text" style="width: 100% !important;" class="form-control" value="'
				+ aData[3] + '">';
			jqTds[3].innerHTML = '<input type="text" style="width: 100% !important;" class="form-control" value="'
				+ aData[4] + '">';
			jqTds[4].innerHTML = '<input type="text" style="width: 100% !important;" class="form-control" value="'
				+ aData[5] + '">';
		...
		function saveRow(oTable, nRow) {
			...
			if( jqInputs[2].value == ""){
				param.priceOfZuslan = 0.0;
			} else {
				param.priceOfZuslan = jqInputs[2].value;
			}
			if( jqInputs[3].value == ""){
				param.priceOfNalaih = 0.0;
			} else {
				param.priceOfNalaih = jqInputs[3].value;
			}
			if( jqInputs[4].value == ""){
				param.priceOfHodoo = 0.0;
			} else {
				param.priceOfHodoo = jqInputs[4].value;
			}
				...
				$.ajax({
					...
					success : function(result) {
						if (result.success == true) {
							...
							oTable.fnUpdate(result.data.priceOfZuslan, nRow, 3, false);
							oTable.fnUpdate(result.data.priceOfNalaih, nRow, 4, false);
							oTable.fnUpdate(result.data.priceOfHodoo, nRow, 5, false);
		...
		var oTable = table.dataTable({
			...
			},
			{
                ...
                render: function ( data, type, row ) {
                	var checked = false;
        			if(data && data.indexOf && data.indexOf('checked="true"') > 0)
        				checked = true;		
        			else if(data === true)
        				checked = true;
		...
		var tableWrapper = $("#cartype_editable_1_wrapper");
