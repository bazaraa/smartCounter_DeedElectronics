var constructionTypeTableEditable = function() {
	...
	var handleTable = function() {
		...
		function editRow(oTable, nRow) {
			...
			jqTds[2].innerHTML = '<input type="checkbox" style="width: 100% !important;" class="form-control input-small" '
					+ checked + '>';
		...
		function saveRow(oTable, nRow) {
			...
			if(param.code == "" || param.name == ""){
				CRSWebUtils.showAlert("danger", "warning", "Төрлийн код эсвэл төрлийн нэрийг оруулаагүй байна. Дахин бүртгэлээ эхлэнэ үү.", 0);
				oTable.fnDeleteRow( nRow, null, true ); 
			} else {
				$.ajax({
					type : "POST",
					url : "/constructiontype",
					async : false,
					dataType : "json",
					data : param,
		...
		function cancelEditRow(oTable, nRow) {
			...
			oTable
					.fnUpdate('<a class="edit" href="">Засах</a>', nRow, 5,
							false);
			oTable.fnDraw();
		}

		var table = $('#constructiontype_editable_1');
		...
		var tableWrapper = $("#constructiontype_editable_1_wrapper");
		...
		$('#constructiontype_editable_1_new')
				.click(
						function(e) {
							e.preventDefault();
