var orderEvaluateForm = function() {
	...
	var handleOrderEvaluateForm = function() {
		...
		$('#orderEvaluate-form').validate({
			...
			focusInvalid : false,
			rules : {
				"evaluateContractFileView.evaluateContractFileType" : {
					required : true
				},
				"evaluateContractFileView.paperSize" : {
					required : true
				},
				"evaluateContractFileView.pageCount" : {
					required : true
				},
				"evaluateContractFileView.respondent" : {
					required : true
				}
			},
...
$("#list_editable_1")
		.on(
				...
				function(e) {
					...
					param.orderContractEvaluateId = $(
							"#orderContractEvaluateId").val();
					param.orderContractEvaluateFileId = rowData["id"];
							...
							.confirm({
								...
								callback : function(result) {
									...
									$
											.ajax({
												type : "POST",
												url : "/orderContractEvaluate/delete",
												async : false,
												dataType : "json",
												data : param,
...
$("#evaluateFileType")
		.change(
				function() {
					...
					$.ajax({
						type : "POST",
						url : "/orderContractEvaluate/getUserName",
						async : false,
						dataType : "json",
						data : param,
						success : function(result) {
...
$(".selectValue").click(
		function() {
			...
			$.ajax({
				type : "POST",
				url : "/orderContractEvaluate/getList",
				async : false,
				dataType : "json",
				data : param,
				success : function(result) {
