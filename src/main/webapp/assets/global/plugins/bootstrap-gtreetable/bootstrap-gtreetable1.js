...
(function($) {
	...
	function GTreeTable(element, options) {
		...
		this.$tree = $(element);
		this.language = this.options.languages[this.options.language] === undefined ? this.options.languages['en-US']
				: $.extend({}, this.options.languages['en-US'],
		...
		this.templateParts = this.options.templateParts !== undefined ? this.options.templateParts
				: {
					draggableIcon : this.options.draggable === true ? '<span class="'
							+ this.options.classes.handleIcon
							+ '">&zwnj;</span><span class="'
							+ this.options.classes.draggablePointer
							+ '">&zwnj;</span>'
							: '',
					indent : '<span class="' + this.options.classes.indent
							+ '">&zwnj;</span>',
					ecIcon : '<span class="' + this.options.classes.ceIcon
							+ ' icon"></span>',
					selectedIcon : '<span class="'
							+ this.options.classes.selectedIcon
							+ ' icon"></span>',
					typeIcon : '<span class="' + this.options.classes.typeIcon
							+ '"></span>',
					name : '<span class="' + this.options.classes.name
							+ '"></span>',
					input : '<input type="text" name="name" value="" style="width: '
							+ this.options.inputWidth
							+ '" class="form-control" />',
					saveButton : '<button type="button" class="btn btn-sm btn-primary '
							+ this.options.classes.saveButton
							+ '">'
							+ lang.save + '</button>',
					cancelButton : '<button type="button" class="btn btn-sm '
							+ this.options.classes.cancelButton + '">'
							+ lang.cancel + '</button>',
					actionsButton : '<button type="button" class="btn btn-sm btn-default dropdown-toggle" data-toggle="dropdown">'
							+ lang.action
		...
		if (this.actions.length > 0) {
			var templateActionsList = '<button type="button"';

			$
					.each(
							this.actions,
							function(index, action) {
									var matches = action.name
											.match(/\$\{([\w\W]+)\}/), name = matches !== null
											&& matches[1] !== undefined
											&& lang.actions[matches[1]] !== undefined ? lang.actions[matches[1]]
											: action.name;
									templateActionsList += 'href="#notarget" class="btn btn-sm btn-default node-action-'
											+ index
											+ '" tabindex="-1">'
											+ name;
							});

			templateActionsList += '</button>';
		...
		this.$nodeTemplate = $(
				this.options.templateSelector !== undefined ? this.options.templateSelector
						: this.options.template).find(
				'.' + this.options.classes.node);
	...
	GTreeTable.prototype = {
		...
		getNodeById : function(id) {
			return this.getNode(this.$tree.find('.' + this.options.classes.node
					+ "[data-id='" + id + "']"));
		},

		getSelectedNodes : function() {
			var selectedNodes = [], that = this;
			$.each(this.$tree.find('.' + this.options.classes.selected),
					function() {
		...
		getSourceNodes : function(nodeId, force) {
			var that = this, oNode = this.getNodeById(nodeId), cached = (nodeId > 0 && this.options.cache > 0);
	...
	GTreeTableNode.prototype = {
		...
		getPath : function() {
			var oNode = this, path = [ oNode.name ], parent = oNode.parent;

			oNode.$node.prevAll('.' + this.manager.options.classes.node).each(
					function() {
		...
		getParents : function() {
			var parents = [], parentId = this.parent;
		...
		getSiblings : function() {
			var oNode = this, siblings = [], findPath = '.'
					+ oNode.manager.options.classes.node + "[data-parent='"
					+ oNode.parent + "']", prev = oNode.$node.prevAll(findPath);
			...
			oNode.$node.nextAll(findPath).each(function() {
		...
		getDescendants : function(options) {
			var oParentNode = this, settings = $.extend({}, {
				depth : 1,
				includeNotSaved : false,
				index : undefined
			}, options), findPath = '.'
					+ oParentNode.manager.options.classes.node, depth = settings.depth !== -1
					|| isNaN(settings.depth) ? settings.depth : Infinity, descendants = [];
			...
			if (depth > 1) {
				oParentNode.$node
						...
						.each(
								function() {
									var oCurrentNode = oParentNode.manager
											.getNode($(this));
									if ((oCurrentNode.level <= oParentNode.level)
											|| (oCurrentNode.level === oParentNode.level && oCurrentNode.parent === oParentNode.parent)) {
										if (!(settings.includeNotSaved === true && !oCurrentNode
												.isSaved())) {
											return false;
										}
									}
				...
				oParentNode.$node.nextAll(
						findPath + "[data-parent='" + oParentNode.id
								+ "'][data-level='" + (oParentNode.level + 1)
								+ "']").each(function() {
			...
			if (!isNaN(settings.index)) {
				var index = settings.index >= 0 ? settings.index - 1
						: descendants.length + settings.index;
		...
		setMovePosition : function(position, pointerOffset) {
			this.$node
					.removeClass(this.manager.options.classes.draggableContainer);
			if (position !== undefined) {
				this.$node
						.addClass(this.manager.options.classes.draggableContainer);
		...
		init : function() {
			...
			this.$name = this.$node.find('.'
					+ this.manager.options.classes.name);
			this.$ceIcon = this.$node.find('.'
					+ this.manager.options.classes.ceIcon);
			this.$typeIcon = this.$node.find('.'
					+ this.manager.options.classes.typeIcon);
			this.$icon = this.$node.find('.'
					+ this.manager.options.classes.icon);
			this.$action = this.$node.find('.'
					+ this.manager.options.classes.action);
			this.$indent = this.$node.find('.'
					+ this.manager.options.classes.indent);
			this.$saveButton = this.$node.find('.'
					+ this.manager.options.classes.saveButton);
			this.$cancelButton = this.$node.find('.'
					+ this.manager.options.classes.cancelButton);
			this.$input = this.$node.find('input');
			this.$pointer = this.$node.find('.'
					+ this.manager.options.classes.draggablePointer);
		...
		render : function() {
			...
			this.$indent
					.css(
							'marginLeft',
							((parseInt(this.level) - this.manager.options.rootLevel) * this.manager.options.nodeIndent)
									+ 'px').html('&zwnj;');

			if (this.type !== undefined && this.manager.options.types
					&& this.manager.options.types[this.type] !== undefined) {
				this.$typeIcon.addClass(this.manager.options.types[this.type])
						.show();
			}
		},

		attachEvents : function() {
			var that = this, selectLimit = parseInt(this.manager.options.selectLimit);

			this.$node.mouseover(function() {
				if (!(that.manager.options.draggable === true && that.manager
						.isNodeDragging() === true)) {
			...
			if (isNaN(selectLimit) === false
					&& (selectLimit > 0 || selectLimit === -1)) {
				this.$name
						.click(function(e) {
							if (that.isSelected()) {
								if ($
										.isFunction(that.manager.options.onUnselect)) {
									that.manager.options.onUnselect(that);
								}
								that.isSelected(false);
							} else {
								var selectedNodes = that.manager
										.getSelectedNodes();
								if (selectLimit === 1
									...
									if ($
											.isFunction(that.manager.options.onSelectOverflow)) {
			...
			$.each(this.manager.actions, function(index, action) {
				that.$node
						.find(
								'.' + that.manager.options.classes.action + '-'
			...
			if (that.manager.options.draggable === true) {
				var getMoveData = function(ui, $droppable) {
					var draggableOffsetTop = ui.offset.top
							- $droppable.offset().top, containerOffsetTop = $droppable
							.offset().top, containerHeight = $droppable
							.outerHeight(), containerWorkspace = containerHeight
							- Math.round(ui.helper.outerHeight() / 2), movePosition, pointerOffset = {
					...
					if (draggableOffsetTop <= (containerWorkspace * 0.3)) {
						...
						pointerOffset.top = containerOffsetTop
								+ (containerWorkspace / 2);
					} else {
						movePosition = 'after';
						pointerOffset.top = containerOffsetTop
								+ containerWorkspace;
				...
				this.$node
						.draggable(
								{
									...
									helper : function(e) {
										var oName = that.manager
												.getNode($(this));
										return '<mark class="'
												+ that.manager.options.classes.draggableHelper
												+ '">' + oName.name + '</mark>';
									},
									cursorAt : {
										top : 0,
									...
									handle : '.'
											+ that.manager.options.classes.handleIcon,
									start : function(e) {
										if (!$.browser.webkit) {
											$(this)
													.data(
															"bs.gtreetable.gtreetablenode.scrollTop",
															$(window)
																	.scrollTop());
										}
									},
									...
									drag : function(e, ui) {
										if (!$.browser.webkit) {
											var strollTop = $(window)
													.scrollTop(), delta = ($(
													this)
													.data(
															"bs.gtreetable.gtreetablenode.scrollTop") - strollTop);

											ui.position.top -= strollTop
													+ delta;
											$(this)
													.data(
															"bs.gtreetable.gtreetablenode.startingScrollTop",
															strollTop);
										}
										var $droppable = $(this)
												.data(
														"bs.gtreetable.gtreetablenode.currentDroppable");
										if ($droppable) {
											var data = getMoveData(ui,
													$droppable);
											that.manager.getNode($droppable)
													.setMovePosition(
															data.position,
															data.pointerOffset);
										}
						...
						.droppable(
								{
									accept : '.'
											+ that.manager.options.classes.node,
									over : function(event, ui) {
										var $this = $(this), data = getMoveData(
												ui, $this);
										that.manager.getNode($this)
												.setMovePosition(data.position,
														data.pointerOffset);
										ui.draggable
												.data(
														"bs.gtreetable.gtreetablenode.currentDroppable",
									...
									out : function(event, ui) {
										ui.draggable
												.removeData("bs.gtreetable.gtreetablenode.currentDroppable");
										that.manager.getNode($(this))
												.setMovePosition();
									},
									drop : function(event, ui) {
										var $this = $(this), oNode = that.manager
												.getNode($this), movePosition = oNode
												.getMovePosition();
										ui.draggable
												.removeData("bs.gtreetable.gtreetablenode.currentDroppable");
										oNode.setMovePosition();
										that.manager.getNode(ui.draggable)
												.move(oNode, movePosition);
									}
								});
			}
		},
		...
		save : function() {
			...
			if ($.isFunction(oNode.manager.options.onSave)) {
				$.when($.ajax(oNode.manager.options.onSave(oNode))).done(
						function(data) {
							oNode._save(data);
						});
			} else {
		...
		_save : function(data) {
			...
			if (this.manager.options.cache > 0) {
				this.manager.cacheManager.synchronize(oNode.isSaved() ? 'edit'
						: 'add', oNode);
			}
		...
		expand : function(options) {
			var oNode = this, prevNode = oNode, settings = $
					.extend(
							{},
							{
								isAltPressed : false,
			...
			$
					.when(
							this.manager.getSourceNodes(oNode.id,
									settings.isAltPressed))
					.done(
							function(result) {
								var data = result[oNode.manager.options.nodesWrapper];
								for ( var x in data) {
									var newNode = new GTreeTableNode(data[x],
											oNode.manager);
								...
								if (settings
										&& typeof $
												.isFunction(settings.onAfterFill)) {
		...
		add : function(position, type) {
			var oTriggerNode = this, childPosition = (position === 'lastChild' || position === 'firstChild'), oNewNode = new GTreeTableNode(
					{
						level : oTriggerNode.level + (childPosition ? 1 : 0),
						parent : oTriggerNode.level === this.manager.options.rootLevel
								&& !childPosition ? 0
								: (childPosition ? oTriggerNode.id
										: oTriggerNode.parent),
						type : type
					}, this.manager), canAddData = this._canAdd(oNewNode);
		...
		insert : function(position, oRelatedNode) {
			var oNode = this, oLastChild, oContext;
			...
			if (position === 'before') {
				...
				this.manager.getNodeById(oRelatedNode.id).$node
						.after(oNode.$node);
		...
		remove : function() {
			...
			if (oNode.isSaved() && $.isFunction(oNode.manager.options.onDelete)) {
				$.when($.ajax(oNode.manager.options.onDelete(oNode))).done(
						function() {
							oNode._remove();
						});
			} else {
		...
		_canMove : function(oDestination, position) {
			...
			if (oDestination.parent === 0
					&& this.manager.options.manyroots === false
					&& position !== 'lastChild') {
				data.result = false;
		...
		move : function(oDestination, position) {
			var oNode = this, moveData = this._canMove(oDestination, position);
			...
			if ($.isFunction(oNode.manager.options.onMove)) {
				$.when(
						$.ajax(oNode.manager.options.onMove(oNode,
								oDestination, position))).done(function(data) {
		...
		_move : function(oDestination, position) {
			var oNode = this, oNodeDescendants = oNode.getDescendants({
				depth : -1,
				includeNotSaved : true
			}), oOldNode = $.extend({}, oNode), oldIP = oNode.getIP(), delta = oDestination.level
					- oNode.level;

			oNode.parent = position === 'lastChild' ? oDestination.id
					: oDestination.parent;
		...
		sort : function() {
			var oNode = this, oSiblings = oNode.getSiblings();

			// nie ma rodzenstwa = sortowanie nie jest potrzebne
			if (oSiblings.length > 0) {
				var oDescendants = !oNode.isExpanded() ? [] : oNode
						.getDescendants({
							depth : -1,
							includeNotSaved : true
						}), oRelated;
				...
				if (oRelated === undefined) {
					...
					if (oRelated.isExpanded()) {
						oRelated = oNode.manager.getNodeById(oNode.parent)
								.getDescendants({
									depth : -1,
									index : -1,
									includeNotSaved : true
		...
		isExpanded : function(action) {
			if (action === undefined) {
				...
				this.$node.addClass(this.manager.options.classes.expanded)
						.removeClass(this.manager.options.classes.collapsed);
				this._isExpanded = true;
			} else {
				this.$node.addClass(this.manager.options.classes.collapsed)
						.removeClass(this.manager.options.classes.expanded);
				this._isExpanded = false;
			}
		},
	...
	GTreeTableCache.prototype = {
		...
		_synchronizeAdd : function(oNode) {
			...
			if (this.manager.options.cache > 1) {
				...
				if (data !== undefined) {
					...
					this.set(oParentNode, this.isSortDefined() ? this
							.sort(data) : data);
				}
			} else {
		...
		_synchronizeEdit : function(oNode) {
			...
			if (this.manager.options.cache > 1) {
				...
				this.set(oParentNode, this.isSortDefined() ? this.sort(data)
						: data);
			} else {
				this.remove(oParentNode);
			}
		...
		_synchronizeDelete : function(oNode) {
			...
			if (this.manager.options.cache > 1) {
				var data = this.get(oParentNode), position;
		...
		_synchronizeMove : function(oNode, params) {
			var that = this, newIP = oNode.getIP(), delta = oNode.level
					- params.oOldNode.level;

			$
					.each(
							this._cached,
							function(index) {
								if (index === params.oldIP
										|| index.indexOf(params.oldIP + '.') === 0) {

									if (that.manager.options.cache > 1) {
										var newData = [], newIndex = index !== params.oldIP ? newIP
												+ index
														.substr(params.oldIP.length)
												: newIP;
	...
	function Plugin(option, _relatedTarget) {
		...
		this
				.each(function() {
					var $this = $(this), data = $this.data('bs.gtreetable'), options = $
							.extend({}, $.fn.gtreetable.defaults, $this.data(),
	...
	$.fn.gtreetable.defaults = {
		...
		defaultActions : [
		/*
		 * { name: '${createBefore}', event: function (oNode, oManager) {
		 * oNode.add('before', 'default'); } }, { name: '${createAfter}', event:
		 * function (oNode, oManager) { oNode.add('after', 'default'); } },
		 */
		{
			name : '${createSelect}',
			event : function(oNode, oManager) {
				$('#myId').val(oNode.getId());
				$('#myName').val(oNode.getName());
